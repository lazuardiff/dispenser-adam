...
void setup()
{
  ...
  myservo.write(180); // Set servo to initial position (180 degrees)
...
void loop()
{
  ...
  float waterHeight = containerHeight - distance1; // Calculate water height

  if (waterHeight <= WATER_LEVEL_THRESHOLD) // if water level is low (about 8 cm)
  {
  ...
  {
    TapState = true;
    myservo.write(90); // Open the tap by rotating servo to 90 degrees
  ...
  {
    TapState = false;
    myservo.write(180); // Close the tap by rotating servo to 180 degrees
